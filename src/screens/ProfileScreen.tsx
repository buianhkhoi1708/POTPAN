...
const ProfileScreen = () => {
  ...
  const { myRecipes, fetchMyRecipes, isLoading: recipeLoading } = useRecipeStore();
  const { myCollections, fetchMyCollections, isLoading: collectionLoading, deleteCollection } = useCollectionStore();
        ...
        { text: t("common.cancel"), style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              ...
              Alert.alert(t("common.error"), t("profile.delete_collection_error"));
            }
          }
        }
      ]
    ...
    <View style={styles.headerContainer}>
      <View style={styles.profileRow}>
        <View style={styles.avatarWrapper}>
          <Image source={{ uri: profile?.avatar_url || "https://i.pravatar.cc/300" }} style={styles.avatar} />
        </View>

        <View style={styles.infoCol}>
          <AppText variant="bold" style={styles.nameText}>{profile?.full_name || t("profile.new_user")}</AppText>
          <AppText variant="medium" style={styles.handleText}>{profile?.username ? `@${profile.username}` : t("profile.no_username")}</AppText>
          <AppText style={styles.bioText} numberOfLines={2}>{profile?.bio || t("profile.no_bio")}</AppText>
        </View>
        ...
        <View style={styles.topRightIcons}>
          ...
          {profile?.role === 'admin' && (
            <Pressable style={[styles.smallIconCircle, { marginLeft: 8, backgroundColor: '#333' }]} onPress={() => navigation.navigate("AdminDashboardScreen")}>
              <Ionicons name="shield-checkmark" size={18} color="#fff" />
            </Pressable>
          )}
          <Pressable style={[styles.smallIconCircle, { marginLeft: 8 }]} onPress={() => navigation.navigate("SettingsScreen")}>
      ...
      <View style={styles.actionButtonsRow}>
        <Pressable style={styles.editButton} onPress={() => navigation.navigate("EditProfileScreen")}>
          ...
          <AppText variant="bold" style={styles.editButtonText}>{t("profile.edit_profile")}</AppText>
        </Pressable>
        <Pressable style={styles.shareButton} onPress={handleShareProfile}>
          ...
          <AppText variant="bold" style={styles.shareButtonText}>{t("profile.share_profile")}</AppText>
        </Pressable>
      </View>

      <View style={styles.statsContainer}>
        <View style={styles.statItem}>
          <AppText variant="bold" style={styles.statNumber}>{myRecipes.length}</AppText>
          <AppText style={styles.statLabel}>{t("profile.recipes_saved")}</AppText>
        </View>
        <View style={styles.verticalDivider} />
        <Pressable style={styles.statItem} onPress={() => navigation.navigate("FollowScreen", { type: "following", userId: user?.id })}>
          <AppText variant="bold" style={styles.statNumber}>{followingCount}</AppText>
          <AppText style={styles.statLabel}>{t("profile.following")}</AppText>
        </Pressable>
        <View style={styles.verticalDivider} />
        <Pressable style={styles.statItem} onPress={() => navigation.navigate("FollowScreen", { type: "followers", userId: user?.id })}>
          <AppText variant="bold" style={styles.statNumber}>{followerCount}</AppText>
          <AppText style={styles.statLabel}>{t("profile.followers")}</AppText>
        </Pressable>
      </View>

      <View style={styles.tabContainer}>
        <Pressable style={[styles.tabItem, activeTab === "recipes" && styles.tabItemActive]} onPress={() => setActiveTab("recipes")}>
          <AppText variant="bold" style={[styles.tabText, activeTab === "recipes" ? styles.tabTextActive : styles.tabTextInactive]}>{t("profile.my_recipes")}</AppText>
        </Pressable>
        <Pressable style={[styles.tabItem, activeTab === "favorites" && styles.tabItemActive]} onPress={() => setActiveTab("favorites")}>
          <AppText variant="bold" style={[styles.tabText, activeTab === "favorites" ? styles.tabTextActive : styles.tabTextInactive]}>{t("profile.my_collections")}</AppText>
        </Pressable>
      </View>
    </View>
  );
  ...
  const renderRecipeItem = ({ item }: { item: any }) => (
    <View style={{ width: CARD_WIDTH, marginBottom: 16 }}>
      <AppRecipeCard item={item} variant="small" onPress={() => navigation.navigate("RecipeDetailScreen", { item })} style={{ width: '100%' }} />
      {item.status === 'pending' && (
  ...
  const renderCollectionItem = ({ item }: { item: any }) => (
    <Pressable style={styles.collectionCard} onPress={() => navigation.navigate("CollectionDetailScreen", { collectionId: item.id, collectionName: item.name })}>
      <View style={styles.imageWrapper}>
        <Image source={{ uri: item.image || "https://images.unsplash.com/photo-1546069901-ba9599a7e63c?w=800&q=80" }} style={styles.collectionImage} resizeMode="cover" />
        <View style={styles.imageOverlay} />
        <View style={styles.iconBadge}><Ionicons name="folder-open" size={20} color={PRIMARY_COLOR} /></View>
        <Pressable style={styles.deleteBtn} onPress={(e) => { e.stopPropagation(); handleDeleteCollection(item.id, item.name); }}>
  ...
  return (
    <AppSafeView style={styles.safeArea}>
      <AppCollectionModal visible={createModalVisible} onClose={() => setCreateModalVisible(false)} recipeId={0} onSaved={onCreateCollectionSuccess} />
      ...
      <FlatList
        ...
        ListHeaderComponent={<>{renderHeader()}{activeTab === 'favorites' && (
        ...
        ListEmptyComponent={
          <View style={{ alignItems: "center", marginTop: 60 }}>
            ...
            <AppText style={{ color: "#999", marginTop: 12 }}>{activeTab === "recipes" ? t("profile.no_recipes") : t("profile.no_collections")}</AppText>
          </View>
        }
      />

      <View style={styles.navBarWrapper}>
        <AppMainNavBar activeTab={activeNavTab} onTabPress={(tab) => { setActiveNavTab(tab); if (tab === "home") navigation.navigate("HomeScreen"); }} />
      </View>
    </AppSafeView>
  );
};
...
const styles = StyleSheet.create({
  ...
  pendingBadge: { position: 'absolute', top: 8, left: 8, backgroundColor: '#FF9800', flexDirection: 'row', alignItems: 'center', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 12, zIndex: 10, shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.2, shadowRadius: 2, elevation: 3 },
