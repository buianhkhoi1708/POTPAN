...
const RecipeDetailScreen = () => {
  ...
  useEffect(() => {
    const fetchAuthor = async () => {
      ...
      const { data } = await supabase.from("users").select("*").eq("id", item.user_id).single();
  ...
  const handleEditPress = () => {
    navigation.navigate("CreateRecipeScreen", { isEdit: true, recipeData: item });
  };

  const handleDeletePress = () => {
    Alert.alert(
        t("alert.delete_recipe_title"),
        t("alert.delete_recipe_msg"),
        [
  ...
  const headerBg = scrollY.interpolate({ inputRange: [0, 100], outputRange: ["rgba(255, 255, 255, 0)", "rgba(255, 255, 255, 1)"], extrapolate: "clamp" });

  const FadingIcon = ({ name, size = 24, colorOverride }: { name: any; size?: number, colorOverride?: string }) => (
    <View style={{ width: size, height: size }}>
      <Ionicons name={name} size={size} color={colorOverride || "#fff"} style={StyleSheet.absoluteFill} />
  ...
  return (
    <AppSafeView style={styles.container}>
      ...
      {/* --- HEADER --- */}
      <Animated.View style={[styles.fixedHeader, { backgroundColor: headerBg, borderBottomWidth: headerOpacity, borderBottomColor: "#eee" }]}>
        <Pressable onPress={() => navigation.goBack()} style={styles.headerBtn}>
          <FadingIcon name="arrow-back" />
        </Pressable>
        <Animated.Text style={[styles.fixedHeaderTitle, { opacity: headerOpacity }]} numberOfLines={1}>
          {item.title}
        </Animated.Text>
      ...
      >
        <View style={styles.imageContainer}>
          <Image source={{ uri: item.thumbnail || "https://via.placeholder.com/400" }} style={styles.image} resizeMode="cover" />
          <View style={styles.imageOverlay} />
        </View>

        <View style={styles.mainInfo}>
          <View style={styles.titleRow}>
            <AppText variant="bold" style={styles.title}>{item.title}</AppText>
            <Pressable
              style={styles.ratingBtn}
              onPress={() => navigation.navigate("ReviewScreen", { recipeId: item.id, recipeTitle: item.title })}
            >
              <Ionicons name="star" size={16} color="#FFC107" />
              <AppText variant="bold" style={styles.statText}>{item.rating ? item.rating.toFixed(1) : "5.0"}</AppText>
          ...
          <View style={styles.authorCard}>
            ...
            {!isMe && (
              <Pressable style={[styles.followBtn, isFollowing && styles.followingBtn]} onPress={() => setIsFollowing(!isFollowing)}>
                <AppText variant="bold" style={[styles.followText, isFollowing && styles.followingText]}>
        ...
        <View style={styles.section}>
          <AppText variant="bold" style={styles.sectionTitle}>{t("recipe_detail.description")}</AppText>
          <AppText style={styles.description}>{item.description || t("recipe_detail.no_description")}</AppText>
        </View>
        ...
        <View style={styles.infoRow}>
            ...
            <View style={styles.infoBadge}>
                 ...
                 <AppText style={styles.infoText}>{formatRecipeTime(item.time, t)}</AppText>
            </View>
            ...
            {/* Danh mục (Dùng trực tiếp key tiếng Việt) */}
             <View style={styles.infoBadge}>
                 ...
                 <AppText style={styles.infoText}>
                    {t(`data_map.category.${item.category}`, { defaultValue: item.category })}
                 </AppText>
            </View>
        </View>
        ...
        <View style={styles.section}>
          <View style={styles.sectionHeaderRow}>
            <AppText variant="bold" style={styles.sectionTitle}>{t("recipe_detail.ingredients")}</AppText>
            <AppText style={styles.itemCount}>{item.ingredients?.length || 0} {t("recipe_detail.items")}</AppText>
          </View>
          <View style={styles.ingredientList}>
            {item.ingredients?.map((ing: any, i: number) => (
              <View key={i} style={styles.ingredientItem}>
                ...
                <AppText variant="bold" style={styles.ingredientAmount}>{ing.quantity || ing.amount}</AppText>
              </View>
            ))}
          </View>
        </View>
        ...
        <View style={styles.section}>
          <View style={styles.sectionHeaderRow}>
            <AppText variant="bold" style={styles.sectionTitle}>{t("recipe_detail.steps")}</AppText>
            <AppText style={styles.itemCount}>{item.steps?.length || 0} {t("recipe_detail.steps_count")}</AppText>
          </View>
          <View style={{ gap: 16 }}>
            {item.steps?.map((step: any, i: number) => (
              <View key={i} style={styles.stepItem}>
                <View style={styles.stepHeader}>
                  ...
                  <AppText variant="bold" style={styles.stepTitle}>{step.title}</AppText>
                </View>
...
const styles = StyleSheet.create({
  ...
  fixedHeader: {
    ...
    justifyContent: "space-between", paddingHorizontal: 16, paddingTop: 50, paddingBottom: 12, zIndex: 100,
  },
  fixedHeaderTitle: { fontSize: 18, color: PRIMARY_COLOR, fontWeight: "700", flex: 1, textAlign: "center" },
  ...
  mainInfo: {
    marginTop: -30, backgroundColor: "#fff", borderTopLeftRadius: 32, borderTopRightRadius: 32,
  ...
  ratingBtn: {
    flexDirection: "row", gap: 4, alignItems: "center", backgroundColor: "#FFF8E1",
    paddingHorizontal: 10, paddingVertical: 6, borderRadius: 16, borderWidth: 1, borderColor: "#FFECB3",
  },
  statText: { color: "#333", fontSize: 14, marginLeft: 2 },
  ratingLinkText: { fontSize: 12, color: "#666" },
  authorCard: {
    flexDirection: "row", alignItems: "center", justifyContent: "space-between",
    backgroundColor: "#FAFAFA", borderRadius: 16, padding: 12, borderWidth: 1, borderColor: "#f0f0f0",
  },
  authorLeft: { flexDirection: "row", alignItems: "center", gap: 12 },
  authorAvatar: { width: 44, height: 44, borderRadius: 22, backgroundColor: "#eee" },
  authorInfo: { gap: 2 },
  authorLabel: { fontSize: 11, color: "#888" },
  authorName: { fontSize: 14, color: "#333" },
  followBtn: { backgroundColor: PRIMARY_COLOR, paddingHorizontal: 16, paddingVertical: 8, borderRadius: 20 },
  followingBtn: { backgroundColor: "#E8F5E9", borderWidth: 1, borderColor: "#4CAF50" },
  ...
  ingredientList: { backgroundColor: "#FAFAFA", borderRadius: 16, padding: 16, borderWidth: 1, borderColor: "#f0f0f0" },
  ingredientItem: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: "#eee" },
  ingredientLeft: { flexDirection: "row", alignItems: "center", flex: 1 },
  bullet: { width: 6, height: 6, borderRadius: 3, backgroundColor: PRIMARY_COLOR, marginRight: 12 },
  ingredientName: { fontSize: 15, color: "#333", flex: 1 },
  ingredientAmount: { color: PRIMARY_COLOR, fontSize: 15 },
  stepItem: { backgroundColor: "#fff", borderRadius: 16, borderWidth: 1, borderColor: "#eee", overflow: "hidden" },
  stepHeader: { flexDirection: "row", alignItems: "center", backgroundColor: "#FDFDFD", paddingHorizontal: 16, paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: "#f5f5f5", gap: 12 },
  stepBadge: { width: 28, height: 28, borderRadius: 14, backgroundColor: PRIMARY_COLOR, alignItems: "center", justifyContent: "center" },
  ...
  cookingFAB: {
    position: "absolute", bottom: 90, left: 24, right: 24, backgroundColor: PRIMARY_COLOR,
    flexDirection: "row", alignItems: "center", justifyContent: "center", paddingVertical: 16,
    borderRadius: 30, gap: 10, shadowColor: PRIMARY_COLOR, shadowOffset: { width: 0, height: 8 },
    shadowOpacity: 0.3, shadowRadius: 12, elevation: 8,
  },
  fabText: { color: "#fff", fontSize: 16, letterSpacing: 0.5 },
  bottomNavWrapper: { position: "absolute", bottom: 0, width: "100%" },
  infoRow: { flexDirection: 'row', paddingHorizontal: 24, marginTop: 16, gap: 10, flexWrap: 'wrap' },
  infoBadge: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#FFF0F0', paddingHorizontal: 10, paddingVertical: 6, borderRadius: 8, gap: 6 },
